interface BankDataEmptyStateProps {
  onReset: () => void;
}

export const BankDataEmptyState = ({ onReset }: BankDataEmptyStateProps) => {
  return (
    <div className="rounded-xl border border-dashed border-[#c3c6d5] bg-white p-7 text-center shadow-sm">
      <h3 className="text-sm font-semibold text-[#191c1e]">No bank data entries found</h3>
      <p className="mt-1.5 text-xs text-[#737784] sm:text-sm">
        Try adjusting your filters or reset all filters to view available entries.
      </p>
      <button
        type="button"
        onClick={onReset}
