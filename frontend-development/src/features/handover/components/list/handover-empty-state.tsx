interface HandoverEmptyStateProps {
  onReset: () => void;
}

export const HandoverEmptyState = ({ onReset }: HandoverEmptyStateProps) => {
  return (
    <div className="rounded-xl border border-dashed border-[#c3c6d5] bg-white p-7 text-center shadow-sm">
      <h3 className="text-sm font-semibold text-[#191c1e]">No handover documents found</h3>
      <p className="mt-1.5 text-xs text-[#737784] sm:text-sm">
        Try adjusting your filters or reset all filters to show handover list.
      </p>
      <button
        type="button"
        onClick={onReset}
