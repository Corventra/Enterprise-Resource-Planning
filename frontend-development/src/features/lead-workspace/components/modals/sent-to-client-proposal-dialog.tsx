interface SentToClientProposalDialogProps {
  open: boolean;
  busy?: boolean;
  onClose: () => void;
...
export const SentToClientProposalDialog = ({
  open,
  busy = false,
  onClose,
  onConfirm
}: SentToClientProposalDialogProps) => {
  if (!open) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4">
      <div className="w-full max-w-md rounded-xl border border-[#eceef0] bg-white p-5 shadow-lg">
        <h2 className="text-base font-semibold text-[#191c1e]">Proposal sudah dikirim ke client?</h2>
        <p className="mt-2 text-sm text-[#737784]">
          Proposal yang ditandai terkirim akan masuk ke tahap menunggu respons client.
        </p>
